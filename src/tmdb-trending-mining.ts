...
async function run() {
    ...
    console.log(`🎬 TMDb Trending Talent Mining — ${MAX_PAGES} pages (${MAX_PAGES * 20} people)\n`);
    ...
    for (let page = 1; page <= MAX_PAGES; page++) {
        const data = await fetchTMDB(`/trending/person/day?language=en-US&page=${page}`);
    ...
    console.log(`\n🔍 Processing ${allPeople.length} trending people...\n`);
