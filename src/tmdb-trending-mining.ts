...
async function run() {
    // HEARTBEAT: START
    await updateWorkflowHeartbeat('Running', 'Fetching trending talent (daily) and performing deep-dive profile scrapes to promote them to hb_talent.');

    console.log('🎬 Starting TMDb Trending Talent Promotion Engine');
    
    // Set a safe limit for trending scrapes
    const maxPages = parseInt(process.env.MAX_PAGES || '3');
    let allPeople: any[] = [];

    for (let page = 1; page <= maxPages; page++) {
        const url = `https://api.themoviedb.org/3/trending/person/day?language=en-US&page=${page}`;
        const response = await fetch(url, {
            headers: {
                Authorization: `Bearer ${TMDB_BEARER_TOKEN}`,
                accept: 'application/json'
    ...
    console.log(`\n🎉 Trending Global Promotion Complete! ${promotedCount}/${allPeople.length} talent profiles processed.\n`);

    // HEARTBEAT: FINISH
    await updateWorkflowHeartbeat('Ready', `Success: ${promotedCount} trending talent discovered and officially promoted to hb_talent.`);
}
